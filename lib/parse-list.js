...
(function () {
  ...
  function parseListPageFromDocument(root) {
    ...
    const blocked = title.includes('403')
      || /erişim\s*engell|access\s*denied/i.test(title)
      || /captcha|güvenlik\s*doğrul|ben\s*robot\s*değilim|erişiminiz\s*engellendi|cf-browser-verification/i.test(bodyText.slice(0, 1500));
    return {
