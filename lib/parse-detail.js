...
(function () {
   ...
   * Dikkat: "güvenlik" / "robot" tek başına false positive üretir
   * (footer, çerez, "güvenli alışveriş"). Sadece net engel sinyalleri.
   */
  function isAccessBlocked(doc) {
    const root = doc || document;
    // Gerçek detay DOM'u varsa engel değildir (footer'daki "güvenlik" yanıltmasın)
    if (root.querySelector('.classifiedDetailTitle, .classifiedInfoList, .classifiedDetailMainPrice, #classifiedDescription')) {
      return false;
    }
    ...
    if (/\b403\b/.test(title) || /erişim\s*engell|access\s*denied/i.test(title)) return true;
    if (/captcha|cloudflare|güvenlik\s*doğrul|ben\s*robot\s*değilim|erişiminiz\s*engellendi/i.test(title)) {
      return true;
    }

    const head = bodyText.slice(0, 1500);
    if (/erişiminiz\s*engellendi|güvenlik\s*doğrulaması|ben\s*robot\s*değilim|access\s*denied|cf-browser-verification|just\s*a\s*moment/i.test(head)) {
      return true;
    }

    if (root.querySelector('#captcha, .captcha, [class*="cf-challenge"], iframe[src*="captcha"], iframe[src*="challenge"]')) {
      return true;
    }
    return false;
  }
